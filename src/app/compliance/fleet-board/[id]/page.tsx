  ...
  return (
    <div className="section-stack">
      <WorkspaceHero
        ...
        subtitle={(
          <>
            <span>{data.vessel.type} • Flag {data.vessel.flag} • IMO {data.vessel.imoNumber ?? "-"}</span>
            <span className="mt-1 block">Principal: {data.vessel.principalName} ({data.vessel.principalCountry})</span>
          </>
        )}
        highlights={[
          {
            label: "Onboard Crew",
            value: data.summary.onboardCrew,
            detail: "Live deployed crew currently assigned to this vessel.",
          },
          {
            label: "Operational Queue",
            value: data.summary.mobilizationCrew,
            detail: "Prepare-joining crew linked to this vessel.",
          },
          {
            label: "Crew With Blockers",
            value: data.summary.crewWithBlockers,
            detail: "Open readiness gaps needing operational follow-up.",
          },
          {
            label: "Missing Rest-Hour Logs",
            value: data.summary.missingRestHourRegisters,
            detail: "No digital rest-hour coverage in the review window.",
          },
        ]}
        helperLinks={[
        ...
        actions={(
          <>
            <Link href="/compliance/fleet-board" className="rounded-full border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-cyan-300 hover:text-cyan-700">
              Fleet Readiness
            </Link>
            <Link href="/compliance/rest-hours" className="rounded-full bg-cyan-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-cyan-800">
              Rest-Hour Register
            </Link>
          </>
        )}
              ...
              {data.onboardCrew.map((member) => (
                <div key={member.crewId} className="px-6 py-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    ...
                    <StatusBadge status={member.restHourStatus} label={formatStatusLabel(member.restHourStatus)} />
                  </div>
              ...
              {data.mobilizationCrew.map((member) => (
                <div key={member.prepareJoiningId} className="px-6 py-4">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    ...
                    <StatusBadge status={member.status} label={formatStatusLabel(member.status)} />
                  </div>
                  <div className="mt-3">
